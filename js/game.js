...
// ... (getTurnOrder fonksiyonu aynı kalır)
...
export async function createBRGame() {
    ...
    const gameData = {
        ...
        status: 'waiting', 
        roundWinner: null, createdAt: new Date(),
        // Eksik olan turnStartTime eklendi
        turnStartTime: firebase.firestore.FieldValue.serverTimestamp(), 
...
export async function joinBRGame(gameId) {
    ...
    try {
        ...
        if (!gameData.players[currentUserId]) {
            // Yeni oyuncuyu BR özellikleriyle ekle
...
export async function joinGame(gameId) {
    ...
    try {
        ...
        // Hata Kontrolü: BR oyununa yanlış butonla katılma kontrolü
...
export async function createBRGame() {
    ...
    try {
        // SET işlemi: Belgeyi oluşturur
        await db.collection("games").doc(gameId).set(gameData);
        // JOIN işlemi: Oluşturucu, otomatik olarak oyuna katılır (zaten players içinde ama dinleyiciyi başlatır)
        await joinBRGame(gameId); 
    } catch (error) {
        console.error("Error creating BR game:", error);
        showToast("BR oyun oluşturulamadı!", true);
...
function updateTurnDisplay(gameData) {
    ...
    if (isBR) {
        ...
        if (gameData.status === 'waiting') {
            ...
            // BR modunda yaratıcı, 2 kişi olduğunda butonu görmeli
